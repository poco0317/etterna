...
WinMoveFileInternal(const std::string& sOldPath, const std::string& sNewPath)
{
	...
	if (!Win9x) {
		if (MoveFileEx(nowide::widen(sOldPath).c_str(),
					   nowide::widen(sNewPath).c_str(),
	...
	if (MoveFile(nowide::widen(sOldPath).c_str(),
				 nowide::widen(sNewPath).c_str()))
		return true;
	...
	if (!DeleteFile(nowide::widen(sNewPath).c_str()))
		return false;

	return !!MoveFile(nowide::widen(sOldPath).c_str(),
					  nowide::widen(sNewPath).c_str());
}
...
WinMoveFile(const std::string& sOldPath, const std::string& sNewPath)
{
	...
	SetFileAttributes(nowide::widen(DoPathReplace(sNewPath)).c_str(),
...
DirectFilenameDB::CacheFile(const std::string& sPath)
{
	...
	WIN32_FIND_DATA fd;
	HANDLE hFind = DoFindFirstFile(nowide::widen(root + sPath).c_str(), &fd);
...
DirectFilenameDB::PopulateFileSet(FileSet& fs, const std::string& path)
{
	...
	HANDLE hFind =
	  DoFindFirstFile(nowide::widen(root + sPath + "/*").c_str(), &fd);
