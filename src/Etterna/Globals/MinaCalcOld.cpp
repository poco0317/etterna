				   ...
				   float score_goal)
{
	...
	if (stream > tech || js > tech || hs > tech)
		if (stream > js && stream > hs)
			stam =
			  Chisel(stream - 0.1f, 2.56f, 1, true, false, true, false, false);
		else if (js > hs)
...
Calc_OLD::TotalMaxPoints()
{
	for (size_t i = 0; i < left_hand.v_itvpoints.size(); i++)
		MaxPoints += static_cast<int>(left_hand.v_itvpoints[i] +
...
vector<float>
MinaSDCalc_OLD(const vector<NoteInfo>& NoteInfo, float musicrate, float goal)
{
	vector<float> o;

	unique_ptr<Calc_OLD> doot = make_unique<Calc_OLD>();
	doot->MusicRate = musicrate;
	CalcClamp(goal, 0.f, ssrcap);
	doot->Scoregoal = goal;
...
MinaSDCalc_OLD(const vector<NoteInfo>& NoteInfo)
{
	...
	if (!NoteInfo.empty()) {
		for (int i = 7; i < rateCount; i++) {
			auto tempVal = MinaSDCalc_OLD(NoteInfo, i / 10.f, 0.93f);
