...
#include "Etterna/Actor/GameplayAndMenus/SnapDisplay.h"
...
enum EditButton
{
	...
	EDIT_BUTTON_SEGMENT_NEXT, /**< Jump to the start of the next segment
								 downward. */
	...
	EDIT_BUTTON_SWITCH_PLAYERS, /**< Allow entering notes for a different
								   Player. */
...
struct MapEditToDI
{
	...
	{
		FOREACH_EditButton(
		  e) for (int slot = 0; slot < NUM_EDIT_TO_DEVICE_SLOTS; ++slot)
		{
...
class ScreenEdit : public ScreenWithMenuElements
{
  ...
  public:
	...
	enum AreaMenuChoice
	{
		paste_at_current_beat, /**< Paste note data starting at the current
								  beat. */
		paste_at_begin_marker, /**< Paste note data starting at the first
								  market. */
		insert_and_shift,
		delete_and_shift,
	...
	enum StepsInformationChoice
	{
		...
		predict_meter, /**< What does the game think this chart's rating should
						  be? */
	...
	void HandleStepsInformationChoice(StepsInformationChoice c,
									  const vector<int>& iAnswers);
	...
	void HandleSongInformationChoice(SongInformationChoice c,
									 const vector<int>& iAnswers);
	...
	void LoadKeymapSectionIntoMappingsMember(XNode const* section,
