...
static Preference<bool> g_bEditorShowBGChangesPlay("EditorShowBGChangesPlay",
												   true);
...
static const char* EditStateNames[] = { "Edit",
										"Record",
										"RecordPaused",
										"Playing" };
XToString(EditState);
...
ScreenEdit::InitEditMappings()
{
	...
	name_to_edit_button["CYCLE_SEGMENT_RIGHT"] =
	  EDIT_BUTTON_CYCLE_SEGMENT_RIGHT;
	...
	name_to_edit_button["SCROLL_NEXT_MEASURE"] =
	  EDIT_BUTTON_SCROLL_NEXT_MEASURE;
	name_to_edit_button["SCROLL_PREV_MEASURE"] =
	...
	name_to_edit_button["INSERT_SHIFT_PAUSES"] =
	  EDIT_BUTTON_INSERT_SHIFT_PAUSES;
	name_to_edit_button["DELETE_SHIFT_PAUSES"] =
	...
	switch (EDIT_MODE.GetValue()) {
		case EditMode_Practice:
			m_EditMappingsDeviceInput
			  .button[EDIT_BUTTON_SCROLL_PREV_MEASURE][0] =
			  DeviceInput(DEVICE_KEYBOARD, KEY_UP);
			m_EditMappingsMenuButton
			  .button[EDIT_BUTTON_SCROLL_PREV_MEASURE][0] = GAME_BUTTON_UP;
			m_EditMappingsDeviceInput
			  .button[EDIT_BUTTON_SCROLL_NEXT_MEASURE][0] =
			  DeviceInput(DEVICE_KEYBOARD, KEY_DOWN);
			m_EditMappingsMenuButton
			  .button[EDIT_BUTTON_SCROLL_NEXT_MEASURE][0] = GAME_BUTTON_DOWN;
			break;
		default:
	...
	switch (EDIT_MODE.GetValue()) {
		...
		case EditMode_Full:
			...
			/* Don't allow F5/F6 in home mode. It breaks the "delay creation
			 * until first save" logic. */
			...
			m_EditMappingsDeviceInput.hold[EDIT_BUTTON_SAMPLE_LENGTH_UP][1] =
			  DeviceInput(DEVICE_KEYBOARD, KEY_RSHIFT);
			m_EditMappingsDeviceInput
			  .button[EDIT_BUTTON_SAMPLE_LENGTH_DOWN][0] =
			  DeviceInput(DEVICE_KEYBOARD, KEY_LBRACKET);
			...
			m_EditMappingsDeviceInput
			  ...
			  DeviceInput(DEVICE_KEYBOARD, KEY_Cb);
			m_EditMappingsDeviceInput
			  .hold[EDIT_BUTTON_OPEN_BGCHANGE_LAYER2_MENU][0] =
			  DeviceInput(DEVICE_KEYBOARD, KEY_LSHIFT);
			m_EditMappingsDeviceInput
			  .hold[EDIT_BUTTON_OPEN_BGCHANGE_LAYER2_MENU][1] =
			  DeviceInput(DEVICE_KEYBOARD, KEY_RSHIFT);
			...
			m_EditMappingsDeviceInput
			  .button[EDIT_BUTTON_INSERT_SHIFT_PAUSES][0] =
			...
			m_EditMappingsDeviceInput.hold[EDIT_BUTTON_INSERT_SHIFT_PAUSES][1] =
			  DeviceInput(DEVICE_KEYBOARD, KEY_RCTRL);
			m_EditMappingsDeviceInput
			  .button[EDIT_BUTTON_DELETE_SHIFT_PAUSES][0] =
	...
	m_EditMappingsDeviceInput
	  .button[EDIT_BUTTON_BAKE_RANDOM_FROM_SONG_GROUP][0] =
	  DeviceInput(DEVICE_KEYBOARD, KEY_Cb);
	...
	m_RecordPausedMappingsDeviceInput
	  .button[EDIT_BUTTON_RECORD_FROM_CURSOR][0] =
	  DeviceInput(DEVICE_KEYBOARD, KEY_Cr);
...
void
ScreenEdit::LoadKeymapSectionIntoMappingsMember(XNode const* section,
												MapEditToDI& mappings)
{
	if (section == NULL) {
		return;
	...
	FOREACH_CONST_Attr(section, attr)
	{
		...
		if (name_entry != name_to_edit_button.end()) {
			...
			for (size_t k = 0;
				 k < key_names.size() && k < NUM_EDIT_TO_DEVICE_SLOTS;
				 ++k) {
				DeviceInput devi;
 ...
 */
EditButton
ScreenEdit::DeviceToEdit(const DeviceInput& DeviceI) const
{
	...
	FOREACH_EditButton(e)
	{
		for (int slot = 0; slot < NUM_EDIT_TO_DEVICE_SLOTS; ++slot) {
			if (pCurrentMap->button[e][slot] == DeviceI &&
				...
				/* The button maps to this function, and has one or more shift
				 * modifiers attached. */
 ...
 */
EditButton
ScreenEdit::MenuButtonToEditButton(GameButton MenuI) const
{
...
bool
ScreenEdit::EditPressed(EditButton button, const DeviceInput& DeviceI)
{
	ASSERT(DeviceI.IsValid());
	...
	/* The button maps to this function.  Does the function has one or more
	 * shift modifiers attached? */
...
bool
ScreenEdit::EditToDevice(EditButton button,
						 int iSlotNum,
...
bool
ScreenEdit::EditIsBeingPressed(EditButton button) const
{
	for (int slot = 0; slot < NUM_EDIT_TO_DEVICE_SLOTS; ++slot) {
		DeviceInput DeviceI;
...
const MapEditButtonToMenuButton*
ScreenEdit::GetCurrentMenuButtonMap() const
{
	switch (m_EditState) {
		DEFAULT_FAIL(m_EditState);
		case STATE_EDITING:
...
int
GetRowsFromAnswers(int choice, const vector<int>& answers);
int
GetRowsFromAnswers(int choice, const vector<int>& answers)
{
	if (answers.empty()) {
...
static MenuDef g_AttackAtTimeMenu("ScreenMiniMenuAttackAtTimeMenu"
								  // fill this in dynamically
);

static MenuDef g_IndividualAttack("ScreenMiniMenuIndividualAttack"
								  // fill this in dynamically
);
...
static MenuDef g_MainMenu("ScreenMiniMenuMainMenu",
...
static MenuDef g_AlterMenu("ScreenMiniMenuAlterMenu",
						   ...
						   MenuRowDef(ScreenEdit::clear,
									  ...
									  0,
									  NULL),
						   MenuRowDef(ScreenEdit::quantize,
									  "Quantize",
									  true,
									  EditMode_Practice,
									  true,
						   ...
						   MenuRowDef(ScreenEdit::turn,
									  ...
									  "Shuffle",
									  "SuperShuffle"),
						   MenuRowDef(ScreenEdit::transform,
									  "Transform",
									  true,
									  EditMode_Practice,
									  true,
						   ...
						   MenuRowDef(ScreenEdit::alter,
									  ...
									  "Shift Right",
									  "Swap Up/Down",
									  "Arbitrary Remap Columns"),
						   MenuRowDef(ScreenEdit::tempo,
									  "Tempo",
									  true,
									  EditMode_Full,
									  true,
...
static MenuDef g_AreaMenu("ScreenMiniMenuAreaMenu",
...
static MenuDef g_StepsInformation("ScreenMiniMenuStepsInformation",
...
static MenuDef g_StepsData(
  ...
  MenuRowDef(ScreenEdit::tap_notes,
			 "Tap Steps",
			 false,
			 EditMode_Full,
			 true,
...
static bool
EnabledIfClipboardTimingIsSafe();
static bool
EnabledIfClipboardTimingIsSafe()
{
	return clipboard_full_timing != NULL;
}
...
static MenuDef g_BackgroundChange(
  ...
  MenuRowDef(ScreenEdit::file1_type,
			 ...
			 "Global BGAnimation",
			 "Global Movie",
			 "Global Movie from Song Group",
			 "Global Movie from Song Group and Genre",
			 "Dynamic Random",
			 "Baked Random",
			 "None"),
...
EnabledIfSet1SongMovie()
{
	return ScreenMiniMenu::s_viLastAnswers[ScreenEdit::file1_type] ==
			 song_movie &&
		   !g_BackgroundChange.rows[ScreenEdit::file1_song_movie]
			  .choices.empty();
}
static bool
EnabledIfSet1SongBitmap()
{
	return ScreenMiniMenu::s_viLastAnswers[ScreenEdit::file1_type] ==
			 song_bitmap &&
		   !g_BackgroundChange.rows[ScreenEdit::file1_song_still]
			  .choices.empty();
}
static bool
EnabledIfSet1GlobalBGAnimation()
...
EnabledIfSet2SongMovie()
{
	return ScreenMiniMenu::s_viLastAnswers[ScreenEdit::file2_type] ==
			 song_movie &&
		   !g_BackgroundChange.rows[ScreenEdit::file2_song_movie]
			  .choices.empty();
}
static bool
EnabledIfSet2SongBitmap()
{
	return ScreenMiniMenu::s_viLastAnswers[ScreenEdit::file2_type] ==
			 song_bitmap &&
		   !g_BackgroundChange.rows[ScreenEdit::file2_song_still]
			  .choices.empty();
}
static bool
EnabledIfSet2GlobalBGAnimation()
...
static std::string
GetOneBakedRandomFile(Song* pSong, bool bTryGenre = true)
{
...
static MenuDef g_InsertTapAttack("ScreenMiniMenuInsertTapAttack",
								 MenuRowDef(-1,
											"Duration seconds",
											true,
											EditMode_Practice,
...
static void
SetDefaultEditorNoteSkin(size_t num,
						 std::string& sNameOut,
...
static Preference1D<std::string> EDITOR_NOTE_SKINS(SetDefaultEditorNoteSkin,
											   NUM_PLAYERS);
...
ScreenEdit::Init()
{
	...
	*/
	...
	// TODO: It would be cleaner to do this by making it possible to set an
	// option in metrics.ini.
	...
	m_soundValueIncrease.Load(THEME->GetPathS("ScreenEdit", "value increase"),
							  true);
	m_soundValueDecrease.Load(THEME->GetPathS("ScreenEdit", "value decrease"),
							  true);
	m_soundSwitchPlayer.Load(THEME->GetPathS("ScreenEdit", "switch player"),
							 true);
	m_soundSwitchTiming.Load(THEME->GetPathS("ScreenEdit", "switch timing"),
							 true);
...
ScreenEdit::~ScreenEdit()
{
	...
	Locator::getLogger()->trace("ScreenEdit::~ScreenEdit()");
...
void
ScreenEdit::PlayPreviewMusic()
{
	SOUND->StopMusic();
	SOUND->PlayMusic(m_pSong->GetPreviewMusicPath(),
					 NULL,
					 false,
...
void
ScreenEdit::MakeFilteredMenuDef(const MenuDef* pDef, MenuDef& menu)
{
	menu = *pDef;
	menu.rows.clear();
...
void
ScreenEdit::EditMiniMenu(const MenuDef* pDef,
...
void
ScreenEdit::Update(float fDeltaTime)
{
	...
	if (m_EditState == STATE_RECORDING) {
			 ...
			 t++) // for each track
		{
			...
			if (GAMESTATE->m_Position.m_fSongBeat <=
				  fStartPlayingAtBeat ||
	...
	if (m_EditState == STATE_RECORDING || m_EditState == STATE_PLAYING) {
		...
		if (bButtonIsBeingPressed && m_EditState == STATE_RECORDING) {
			...
			fLastBeat = m_pSteps->GetTimingData()->GetBeatFromElapsedTime(
			  fSeconds + 0.5f);
		}

		float fStopAtSeconds =
	...
	if (fabsf(fDelta) < 10)
		...
		fapproach(m_fTrailingBeat, GetBeat(), fabsf(fDelta) * fDeltaTime * 5);
...
static ThemeMetric<std::string> CURRENT_BEAT_FORMAT("ScreenEdit",
												"CurrentBeatFormat");
static ThemeMetric<std::string> CURRENT_SECOND_FORMAT("ScreenEdit",
												  "CurrentSecondFormat");
...
static ThemeMetric<std::string> DESCRIPTION_FORMAT("ScreenEdit",
											   "DescriptionFormat");
static ThemeMetric<std::string> CHART_STYLE_FORMAT("ScreenEdit",
											   "ChartStyleFormat");
static ThemeMetric<std::string> STEP_AUTHOR_FORMAT("ScreenEdit",
											   "StepAuthorFormat");
...
static ThemeMetric<std::string> TAP_NOTE_TYPE_FORMAT("ScreenEdit",
												 "TapNoteTypeFormat");
static ThemeMetric<std::string> SEGMENT_TYPE_FORMAT("ScreenEdit",
												"SegmentTypeFormat");
...
static ThemeMetric<std::string> TIMING_MODE_FORMAT("ScreenEdit",
											   "TimingModeFormat");
static ThemeMetric<std::string> BEAT_0_OFFSET_FORMAT("ScreenEdit",
												 "Beat0OffsetFormat");
static ThemeMetric<std::string> PREVIEW_START_FORMAT("ScreenEdit",
												 "PreviewStartFormat");
static ThemeMetric<std::string> PREVIEW_LENGTH_FORMAT("ScreenEdit",
												  "PreviewLengthFormat");
static ThemeMetric<std::string> RECORD_HOLD_TIME_FORMAT("ScreenEdit",
													"RecordHoldTimeFormat");
void
ScreenEdit::UpdateTextInfo()
{
	if (m_pSteps == NULL)
		return;
	...
	std::string sNoteType =
	  ssprintf(NOTES.GetValue(),
			   NoteTypeToLocalizedString(m_SnapDisplay.GetNoteType()).c_str());

	std::string sText;
	sText += ssprintf(CURRENT_BEAT_FORMAT.GetValue(),
					  CURRENT_BEAT.GetValue().c_str(),
					  GetBeat());
	float second =
	  GetAppropriateTiming().GetElapsedTimeFromBeatNoOffset(GetBeat());
	sText += ssprintf(CURRENT_SECOND_FORMAT.GetValue(),
					  CURRENT_SECOND.GetValue().c_str(),
					  second);
	switch (EDIT_MODE.GetValue()) {
		...
		case EditMode_Home:
		case EditMode_Full:
			sText += ssprintf(SNAP_TO_FORMAT.GetValue(),
							  SNAP_TO.GetValue().c_str(),
							  sNoteType.c_str());
			break;
	}

	if (m_NoteFieldEdit.m_iBeginMarker != -1) {
		sText += ssprintf(SELECTION_BEAT_BEGIN_FORMAT.GetValue(),
	...
	if (EDIT_MODE.GetValue() == EditMode_Full) {
		...
		sText += ssprintf(CHART_NAME_FORMAT.GetValue(),
						  CHART_NAME.GetValue().c_str(),
						  m_pSteps->GetChartName().c_str());
		sText += ssprintf(STEP_AUTHOR_FORMAT.GetValue(),
						  STEP_AUTHOR.GetValue().c_str(),
		...
		sText += ssprintf(MAIN_TITLE_FORMAT.GetValue(),
						  MAIN_TITLE.GetValue().c_str(),
						  m_pSong->m_sMainTitle.c_str());
		if (m_pSong->m_sSubTitle.size())
			sText += ssprintf(SUBTITLE_FORMAT.GetValue(),
							  SUBTITLE.GetValue().c_str(),
							  m_pSong->m_sSubTitle.c_str());
		sText +=
		  ssprintf(SEGMENT_TYPE_FORMAT.GetValue(),
	...
	switch (EDIT_MODE.GetValue()) {
		...
		case EditMode_Full:
			...
			sText += ssprintf(PREVIEW_START_FORMAT.GetValue(),
							  PREVIEW_START.GetValue().c_str(),
							  m_pSong->m_fMusicSampleStartSeconds);
			sText += ssprintf(PREVIEW_LENGTH_FORMAT.GetValue(),
							  PREVIEW_LENGTH.GetValue().c_str(),
							  m_pSong->m_fMusicSampleLengthSeconds);
			if (record_hold_seconds < record_hold_default - .001f ||
				record_hold_seconds > record_hold_default + .001f) {
				sText += ssprintf(RECORD_HOLD_TIME_FORMAT.GetValue(),
								  RECORD_HOLD_TIME.GetValue().c_str(),
								  record_hold_seconds);
			}
			break;
	}
...
bool
ScreenEdit::Input(const InputEventPlus& input)
{
...
static void
ShiftToRightSide(int& iCol, int iNumTracks)
{
...
static LocalizedString BG_CHANGE_STEP_TIMING(
  "ScreenEdit",
  "You must be in Song Timing Mode to edit BG Changes.");
static LocalizedString ALTER_MENU_NO_SELECTION(
  "ScreenEdit",
  "You must have an area selected to enter the Alter Menu.");
static LocalizedString SWITCHED_TO("ScreenEdit", "Switched to");
static LocalizedString NO_BACKGROUNDS_AVAILABLE("ScreenEdit",
												"No backgrounds available");
static ThemeMetric<bool> INVERT_SCROLL_BUTTONS("ScreenEdit",
											   "InvertScrollSpeedButtons");

bool
ScreenEdit::InputEdit(const InputEventPlus& input, EditButton EditB)
{
	...
	switch (EditB) {
		...
		case EDIT_BUTTON_COLUMN_9: {
			...
			if (iCol >= m_NoteDataEdit.GetNumTracks())
				return false; // this button is not in the range of columns for
							  // this Style
			...
			if (m_NoteDataEdit.IsHoldNoteAtRow(iCol, iSongIndex, &iHeadRow)) {
				...
				// Don't CheckNumberOfNotesAndUndo.  We don't want to revert any
				// change that removes notes.
			} else if (m_NoteDataEdit.GetTapNote(iCol, iSongIndex).type !=
					   TapNoteType_Empty) {
				m_soundRemoveNote.Play(true);
				SetDirty(true);
				SaveUndo();
		...
		case EDIT_BUTTON_SCROLL_NEXT_MEASURE: {
			...
			fDestinationBeat =
			  ftruncf(fDestinationBeat, (float)beatsPerMeasure);
			ScrollTo(fDestinationBeat);
			return true;
		}
		case EDIT_BUTTON_SCROLL_PREV_MEASURE: {
			float fDestinationBeat =
			  QuantizeUp(GetBeat(), (float)beatsPerMeasure);
		...
		case EDIT_BUTTON_OPEN_AREA_MENU: {
			// TODO: Improve behavior if timing changes are shifted down on beat
			// 0.
			g_AreaMenu.rows[shift_pauses_forward].bEnabled = (GetBeat() != 0);
			g_AreaMenu.rows[paste_at_current_beat].bEnabled =
			  !m_Clipboard.IsEmpty();
			g_AreaMenu.rows[paste_at_begin_marker].bEnabled =
			  !m_Clipboard.IsEmpty() != 0 &&
			  m_NoteFieldEdit.m_iBeginMarker != -1;
		...
		case EDIT_BUTTON_OPEN_PREV_STEPS: {
			...
			// Reload the music because it can be different for every steps.
			// -Kyz
			m_AutoKeysounds.FinishLoading();
			m_pSoundMusic = m_AutoKeysounds.GetSound();
		...
		case EDIT_BUTTON_SAMPLE_LENGTH_UP: {
			...
			if (EditB == EDIT_BUTTON_SAMPLE_LENGTH_DOWN ||
				...
				m_pSong->m_fMusicSampleLengthSeconds =
				  std::max(m_pSong->m_fMusicSampleLengthSeconds, 0.f);
			} else {
				m_pSong->m_fMusicSampleStartSeconds += fDelta;
				m_pSong->m_fMusicSampleStartSeconds =
				  std::max(m_pSong->m_fMusicSampleStartSeconds, 0.f);
			}
			(fDelta > 0 ? m_soundValueIncrease : m_soundValueDecrease)
			  .Play(true);
			SetDirty(true);
		...
		case EDIT_BUTTON_OPEN_BGCHANGE_LAYER2_MENU:
			if (!GAMESTATE->m_bIsUsingStepTiming) {
...

				{
					...
					menu.rows[layer].choices[0] =
					  ssprintf("%d", g_CurrentBGChangeLayer);
					...
					BackgroundUtil::GetGlobalBGAnimations(
					  ...
					  menu.rows[file1_global_bganimation]
						.choices); // NULL to get all background files

					menu.rows[file2_type].choices =
					  menu.rows[file1_type].choices;
					...
					menu.rows[file2_global_movie_song_group_and_genre].choices =
					  menu.rows[file1_global_movie_song_group_and_genre]
						.choices;
...
#undef FILL_ENABLED
					...
					menu.rows[effect].SetDefaultChoiceIfPresent(
					  bgChange.m_def.m_sEffect);
					...
					if (menu.rows[file1_song_bganimation]
					...
					if (menu.rows[file1_global_bganimation]
						  .SetDefaultChoiceIfPresent(bgChange.m_def.m_sFile1))
					...
					if (menu.rows[file2_song_bganimation]
					...
					if (menu.rows[file2_global_bganimation]
						  .SetDefaultChoiceIfPresent(bgChange.m_def.m_sFile2))
					...
					if (menu.rows[file2_global_movie_song_group_and_genre]
						...
						menu.rows[file2_type].iDefaultChoice =
						  global_movie_song_group_and_genre;
					menu.rows[color1].SetDefaultChoiceIfPresent(
					  bgChange.m_def.m_sColor1);
					menu.rows[color2].SetDefaultChoiceIfPresent(
					  bgChange.m_def.m_sColor2);
		...
		case EDIT_BUTTON_BAKE_RANDOM_FROM_SONG_GROUP_AND_GENRE: {
			...
			if (sName.empty()) {
				...
				FOREACH(
				  BackgroundChange, m_pSong->GetBackgroundChanges(iLayer), bgc)
				{
					if (bgc->m_fStartBeat ==
						GAMESTATE->m_Position.m_fSongBeat) {
						bgChange = *bgc;
			...
			if (m_NoteFieldEdit.m_iBeginMarker != -1 &&
				...
				if (g_iDefaultRecordLength.Get() == -1) {
					...
					m_iStartPlayingAt = BeatToNoteRow(
					  ftruncf(GetAppropriatePosition().m_fSongBeat,
...
bool
ScreenEdit::InputRecord(const InputEventPlus& input, EditButton EditB)
{
	...
	switch (input.type) {
		...
		case IET_FIRST_PRESS: {
			...
			float fBeat = GetBeat();
			fBeat =
			  Quantize(fBeat, NoteTypeToBeat(m_SnapDisplay.GetNoteType()));
...
bool
ScreenEdit::InputRecordPaused(const InputEventPlus& input, EditButton EditB)
{
	if (input.type != IET_FIRST_PRESS)
	...
	switch (EditB) {
		case EDIT_BUTTON_UNDO:
			/* We've already actually committed changes to m_NoteDataEdit, so
			 * all we have to do to undo is Undo() as usual, and copy the note
			 * data back in. */
			Undo();
...
bool
ScreenEdit::InputPlay(const InputEventPlus& input, EditButton EditB)
{
	switch (input.type) {
		case IET_RELEASE:
		case IET_FIRST_PRESS:
	...
	if (gbt == GameButtonType_Menu && input.type == IET_FIRST_PRESS) {
		switch (EditB) {
			case EDIT_BUTTON_RETURN_TO_EDIT:
				/* When exiting play mode manually, leave the cursor where it
				 * is. */
			...
			case EDIT_BUTTON_OFFSET_DOWN: {
				...
				GetAppropriateTimingForUpdate().m_fBeat0OffsetInSeconds +=
				  fOffsetDelta;
				if (!GAMESTATE->m_bIsUsingStepTiming) {
...
void
ScreenEdit::TransitionEditState(EditState em)
{
	...
	if (bStateChanging) {
		switch (old) {
			...
			case STATE_RECORDING:
				...
				m_NoteDataEdit.CopyRange(m_NoteDataRecord,
										 m_iStartPlayingAt,
										 m_iStopPlayingAt,
										 m_iStartPlayingAt);
	...
	switch (em) {
		DEFAULT_FAIL(em);
		case STATE_EDITING:
			// Important: people will stop playing, change the BG and start
			// again; make sure we reload
			m_Background.Unload();
			m_Foreground.Unload();

			// Restore the cursor position + Quantize + Clamp
			SetBeat(std::max(0.f,
						Quantize(m_fBeatToReturnTo,
		...
		case STATE_RECORDING: {
			...
			/* Give a lead-in.  If we're loading Player, this must be done
	...
	// Play the OnCommands again so that these will be re-hidden if the
	// OnCommand hides them.
	if (em == STATE_EDITING) {
	...
	m_NoteFieldRecord.SetVisible(em == STATE_RECORDING ||
								 em == STATE_RECORDING_PAUSED);
	...
	switch (em) {
		...
		case STATE_RECORDING: {
			const float fStartSeconds =
			  GetAppropriateTiming().GetElapsedTimeFromBeat(GetBeat());
			Locator::getLogger()->trace("Starting playback at %f", fStartSeconds);
...
void
ScreenEdit::ScrollTo(float fDestinationBeat)
{
	...
	for (int n = 0; n < NUM_EDIT_BUTTON_COLUMNS; n++) {
		...
		// segment that the user made.  Dragging the hold bigger can only absorb
		// and remove other taps, so dragging won't cause us to exceed the note
		// limit.
		TapNote tn = EditIsBeingPressed(EDIT_BUTTON_LAY_ROLL)
					   ? TAP_ORIGINAL_ROLL_HEAD
					   : TAP_ORIGINAL_HOLD_HEAD;
	...
	if (EditIsBeingPressed(EDIT_BUTTON_SCROLL_SELECT)) {
		...
		if (iDestinationRow == m_iShiftAnchor) {
			...
			if (m_NoteFieldEdit.m_iBeginMarker > m_NoteFieldEdit.m_iEndMarker)
				std::swap(m_NoteFieldEdit.m_iBeginMarker,
					 m_NoteFieldEdit.m_iEndMarker);
		}
	}
...
static LocalizedString NEW_KEYSOUND_FILE("ScreenEdit",
										 "Enter New Keysound File");

void
ScreenEdit::HandleMessage(const Message& msg)
{
	if (msg == "Judgment") {
		PlayerNumber pn;
...
static LocalizedString AUTOSAVE_SUCCESSFUL("ScreenEdit",
										   "Autosave successful.");
static LocalizedString SAVE_SUCCESS_NO_SM_SPLIT_TIMING(
  "ScreenEdit",
  "save_success_no_sm_split_timing");
...
void
ScreenEdit::HandleScreenMessage(const ScreenMessage SM)
{
	...
	if (SM == SM_UpdateTextInfo) {
		...
		HandleMainMenuChoice((MainMenuChoice)ScreenMiniMenu::s_iLastRowCode,
		...
		HandleAlterMenuChoice((AlterMenuChoice)ScreenMiniMenu::s_iLastRowCode,
							  ScreenMiniMenu::s_viLastAnswers);
		...
		if (sscanf(ScreenTextEntry::s_sLastAnswer.c_str(),
				   " %d / %d ",
				   &iCombo,
				   &iMiss) == 2)
			GetAppropriateTimingForUpdate().AddSegment(
		...
		if (!GetAppropriateTiming().DoesLabelExist(sLabel)) {
			...
			size_t s = 0;
			while ((s = sLabel.find("=", s)) != std::string::npos) {
				sLabel.replace(s, std::string("=").length(), "_");
				s += std::string("_").length();
			}
			s = 0;
			while ((s = sLabel.find(",", s)) != std::string::npos) {
				sLabel.replace(s, std::string(",").length(), "_");
		...
		} else if (ScreenTextEntry::s_sLastAnswer.substr(0, 1) == "s" ||
		...
		{
			GetAppropriateTimingForUpdate().AddSegment(
			  FakeSegment(GetRow(), fFake));
			SetDirty(true);
		}
	} else if (SM == SM_BackFromStepMusicChange &&
			   !ScreenTextEntry::s_bCancelledLast) {
		// Reload the music because it just changed. -Kyz
		m_AutoKeysounds.FinishLoading();
		m_pSoundMusic = m_AutoKeysounds.GetSound();
	} else if (SM == SM_BackFromBGChange) {
		HandleBGChangeChoice((BGChangeChoice)ScreenMiniMenu::s_iLastRowCode,
							 ScreenMiniMenu::s_viLastAnswers);
		SetDirty(true);
		...
		if (track < tracks) {
			...
			if (sound < kses.size()) {
				...
				if (newNote.type == TapNoteType_Empty) {
					newNote.type =
					  TapNoteType_AutoKeysound; // keysounds need something non
												// empty.
				}
			} else // sound > kses.size()
			{
				...
				if (newNote.type == TapNoteType_AutoKeysound) {
					newNote.type = TapNoteType_Empty; // autoKeysound with no
													  // sound is pointless.
				}
			}
			...
			for (int t = 0; t < tracks; ++t) {
				FOREACH_NONEMPTY_ROW_IN_TRACK(m_NoteDataEdit, t, r)
				{
					...
					TapNote newNote = oldNote; // need to lose the const. not
		...
		SetDirty(true);
	} else if (SM == SM_BackFromNewKeysound &&
			   !ScreenTextEntry::s_bCancelledLast) {
		...
		if (ScreenPrompt::s_LastAnswer == ANSWER_YES) {
			m_NoteDataEdit.ClearRange(m_NoteFieldEdit.m_iBeginMarker,
									  m_NoteFieldEdit.m_iEndMarker);
		}
									   ...
									   // no, cancel"
	{
		...
		Locator::getLogger()->trace("Save successful.");
		CopyToLastSave();
		SetDirty(false);
		...
		Locator::getLogger()->trace("AutoSave successful.");
	...
	} else if (SM == SM_SaveFailed) // save failed; stay in the editor
	{
		...
		Locator::getLogger()->trace("Save failed. Changes uncommitted from memory.");
		CopyFromLastSave();
		...
		/* When another screen comes up, RageSounds takes over the sound timer.
		 * When we come back, put the timer back to where it was. */
...
void
ScreenEdit::SetDirty(bool dirty)
{
	if (EDIT_MODE.GetValue() != EditMode_Full) {
		m_dirty = false;
...
void
ScreenEdit::PerformSave(bool autosave)
{
	...
	switch (EDIT_MODE.GetValue()) {
		...
		case EditMode_Home: {
			...
			std::string sError;
			m_pSteps->CalculateRadarValues();

			Locator::getLogger()->warn("hol up");
			break;
			/*
			...
			*/
			...
			// This only works because important non-steps data can't be changed
			// in home mode (BPMs, stops).
			ClearUndo();
			...
			/* FIXME
				 ...
				 switch( c )
				 {
				 case save:			s = "ScreenMemcardSaveEditsAfterSave";
			   break;
				 case save_on_exit:	s = "ScreenMemcardSaveEditsAfterExit";
			   break; default:		FAIL_M(ssprintf("Invalid menu choice: %i",
			   c));
				 }
...
static void
ChangeDescription(const std::string& sNew)
{
	Steps* pSteps = GAMESTATE->m_pCurSteps;
...
static void
ChangeChartName(const std::string& sNew)
{
	Steps* pSteps = GAMESTATE->m_pCurSteps;
...
static void
ChangeChartStyle(const std::string& sNew)
{
	Steps* pSteps = GAMESTATE->m_pCurSteps;
...
static void
ChangeStepCredit(const std::string& sNew)
{
	Steps* pSteps = GAMESTATE->m_pCurSteps;
...
static void
ChangeStepMeter(const std::string& sNew)
{
	int diff = StringToInt(sNew);
	GAMESTATE->m_pCurSteps->SetMeter(std::max(diff, 1));
}

static void
ChangeStepMusic(const std::string& sNew)
{
	Steps* pSteps = GAMESTATE->m_pCurSteps;
...
static void
ChangeMainTitle(const std::string& sNew)
{
	Song* pSong = GAMESTATE->m_pCurSong;
...
static void
ChangeSubTitle(const std::string& sNew)
{
	Song* pSong = GAMESTATE->m_pCurSong;
...
static void
ChangeArtist(const std::string& sNew)
{
	Song* pSong = GAMESTATE->m_pCurSong;
...
static void
ChangeGenre(const std::string& sNew)
{
	Song* pSong = GAMESTATE->m_pCurSong;
	pSong->m_sGenre = sNew;
}
...
static void
ChangeCredit(const std::string& sNew)
{
	Song* pSong = GAMESTATE->m_pCurSong;
...
static void
ChangePreview(const std::string& sNew)
{
	Song* pSong = GAMESTATE->m_pCurSong;
	if (!sNew.empty()) {
		std::string error;
		...
		if (sample == NULL) {
			Locator::getLogger()->trace("Preview file",
						 pSong->GetPreviewMusicPath(),
...
static void
ChangeMainTitleTranslit(const std::string& sNew)
{
	Song* pSong = GAMESTATE->m_pCurSong;
...
static void
ChangeSubTitleTranslit(const std::string& sNew)
{
	Song* pSong = GAMESTATE->m_pCurSong;
...
static void
ChangeArtistTranslit(const std::string& sNew)
{
	Song* pSong = GAMESTATE->m_pCurSong;
...
static void
ChangeLastSecondHint(const std::string& sNew)
{
...
static void
ChangeMaxBPM(const std::string& sNew)
{
	GAMESTATE->m_pCurSong->m_fSpecifiedBPMMax = StringToFloat(sNew);
}

static void
ChangeStepsMaxBPM(const std::string& sNew)
{
	Steps* step = GAMESTATE->m_pCurSteps;
...
const TimingData&
ScreenEdit::GetAppropriateTiming() const
{
	if (GAMESTATE->m_bIsUsingStepTiming) {
...
TimingData&
ScreenEdit::GetAppropriateTimingForUpdate()
{
	if (GAMESTATE->m_bIsUsingStepTiming) {
...
inline void
ScreenEdit::SetBeat(float fBeat)
{
...
ScreenEdit::DisplayTimingMenu()
{
	...
	g_TimingDataInformation.rows[warp].SetOneUnthemedChoice(
	  FloatToString(pTime.GetWarpAtRow(row)));
	g_TimingDataInformation.rows[speed_percent].SetOneUnthemedChoice(
	  bHasSpeedOnThisRow ? FloatToString(pTime.GetSpeedPercentAtRow(row))
						 : "---");
	g_TimingDataInformation.rows[speed_wait].SetOneUnthemedChoice(
	  bHasSpeedOnThisRow ? FloatToString(pTime.GetSpeedWaitAtRow(row)) : "---");
	...
	g_TimingDataInformation.rows[scroll].SetOneUnthemedChoice(
	  FloatToString(pTime.GetScrollAtRow(row)));
	g_TimingDataInformation.rows[fake].SetOneUnthemedChoice(
	  FloatToString(pTime.GetFakeAtRow(row)));
...
void
ScreenEdit::DisplayTimingChangeMenu()
{
	int row = GetRow();
	EditMiniMenu(&g_TimingDataChangeInformation,
				 SM_BackFromTimingDataChangeInformation);
}
...
static LocalizedString REVERT_FROM_DISK("ScreenEdit",
...
int
ScreenEdit::GetSongOrNotesEnd()
{
	return std::max(
	  m_iStartPlayingAt,
	  std::max(m_NoteDataEdit.GetLastRow(),
		  BeatToNoteRow(m_pSteps->GetTimingData()->GetBeatFromElapsedTime(
			GAMESTATE->m_pCurSong->m_fMusicLengthSeconds))));
}

void
ScreenEdit::HandleMainMenuChoice(MainMenuChoice c, const vector<int>& iAnswers)
{
	GAMESTATE->SetProcessedTimingData(m_pSteps->GetTimingData());
	switch (c) {
		DEFAULT_FAIL(c);
		...
		case play_current_beat_to_end: {
			m_iStartPlayingAt =
			  BeatToNoteRow(GAMESTATE->m_Position.m_fSongBeat);
			m_iStopPlayingAt = GetSongOrNotesEnd();
			TransitionEditState(STATE_PLAYING);
		} break;
			...
			FOREACH_ENUM(Difficulty, dc)
			{
				g_StepsInformation.rows[difficulty].choices.push_back(
				  "|" + CustomDifficultyToLocalizedString(
						  GetCustomDifficulty(pSteps->m_StepsType, dc)));
			}
		...
		case edit_song_info: {
			...
			g_SongInformation.rows[sub_title].SetOneUnthemedChoice(
			  pSong->m_sSubTitle);
			g_SongInformation.rows[artist].SetOneUnthemedChoice(
			  pSong->m_sArtist);
			g_SongInformation.rows[genre].SetOneUnthemedChoice(pSong->m_sGenre);
			g_SongInformation.rows[credit].SetOneUnthemedChoice(
			  pSong->m_sCredit);
			g_SongInformation.rows[preview].SetOneUnthemedChoice(
			  pSong->m_PreviewFile);
			g_SongInformation.rows[main_title_transliteration]
			  .SetOneUnthemedChoice(pSong->m_sMainTitleTranslit);
			g_SongInformation.rows[sub_title_transliteration]
			  .SetOneUnthemedChoice(pSong->m_sSubTitleTranslit);
			...
			g_SongInformation.rows[last_second_hint].SetOneUnthemedChoice(
			  FloatToString(pSong->GetSpecifiedLastSecond()));
			g_SongInformation.rows[preview_start].SetOneUnthemedChoice(
			  FloatToString(pSong->m_fMusicSampleStartSeconds));
			g_SongInformation.rows[preview_length].SetOneUnthemedChoice(
			  FloatToString(pSong->m_fMusicSampleLengthSeconds));
			...
			g_SongInformation.rows[min_bpm].SetOneUnthemedChoice(
			  FloatToString(pSong->m_fSpecifiedBPMMin));
			g_SongInformation.rows[max_bpm].SetOneUnthemedChoice(
			  FloatToString(pSong->m_fSpecifiedBPMMax));
		...
		case exit:
			switch (EDIT_MODE.GetValue()) {
				...
				case EditMode_Home:
					if (IsDirty())
						ScreenPrompt::Prompt(SM_DoSaveAndExit,
											 SAVE_CHANGES_BEFORE_EXITING,
...
static LocalizedString ENTER_ARBITRARY_MAPPING("ScreenEdit",
											   "Enter the new track mapping.");
static LocalizedString TOO_MANY_TRACKS("ScreenEdit",
									   "Too many tracks specified.");
static LocalizedString NOT_A_TRACK("ScreenEdit", "'%s' is not a track id.");
static LocalizedString OUT_OF_RANGE_ID(
  "ScreenEdit",
  "Entry %d, '%d', is out of range 1 to %d.");
static LocalizedString CONFIRM_CLEAR(
  "ScreenEdit",
  "Are you sure you want to clear %d notes?");

static bool
ConvertMappingInputToMapping(std::string const& mapstr,
							 int* mapping,
							 std::string& error)
{
	vector<std::string> mapping_input;
	...
	for (; track < mapping_input.size(); ++track) {
		if (mapping_input[track].empty() || mapping_input[track] == " ") {
			...
			error =
			  ssprintf(NOT_A_TRACK.GetValue(), mapping_input[track].c_str());
			return false;
		}
		if (mapping[track] < 1 ||
			mapping[track] > static_cast<int>(tracks_for_type)) {
			error = ssprintf(OUT_OF_RANGE_ID.GetValue(),
							 track + 1,
							 mapping[track],
							 tracks_for_type);
			return false;
...
static bool
ArbitraryRemapValidate(const std::string& answer, std::string& error_out)
{
...
void
ScreenEdit::HandleArbitraryRemapping(std::string const& mapstr)
{
...
void
ScreenEdit::HandleAlterMenuChoice(AlterMenuChoice c,
								  const vector<int>& answers,
								  bool allow_undo,
								  bool prompt_clear)
{
	ASSERT_M(m_NoteFieldEdit.m_iBeginMarker != -1 &&
			   m_NoteFieldEdit.m_iEndMarker != -1,
			 "You can only alter a selection of notes with a selection to "
			 "begin with!");
	...
	switch (c) {
		...
		case copy: {
			...
			m_Clipboard.CopyRange(m_NoteDataEdit,
								  m_NoteFieldEdit.m_iBeginMarker,
								  m_NoteFieldEdit.m_iEndMarker);
		} break;
		case clear: {
		...
		case quantize: {
			...
			NoteDataUtil::SnapToNearestNoteType(m_NoteDataEdit,
												nt,
												nt,
												m_NoteFieldEdit.m_iBeginMarker,
												m_NoteFieldEdit.m_iEndMarker);
		...
		case turn: {
			...
			switch (tt) {
				...
				case turn_backwards:
					NoteDataUtil::Turn(
					  m_Clipboard, st, NoteDataUtil::backwards);
					break;
				case shuffle:
				...
				case super_shuffle:
					NoteDataUtil::Turn(
					  m_Clipboard, st, NoteDataUtil::super_shuffle);
					break;
			}
		...
		case transform: {
			...
			switch (tt) {
				DEFAULT_FAIL(tt);
				case noholds:
					NoteDataUtil::RemoveHoldNotes(
					  m_NoteDataEdit, iBeginRow, iEndRow);
					break;
				case nomines:
					NoteDataUtil::RemoveMines(
					  m_NoteDataEdit, iBeginRow, iEndRow);
					break;
				case little:
				...
				case twister:
					...
					break;
				case nojumps:
					NoteDataUtil::RemoveJumps(
					  m_NoteDataEdit, iBeginRow, iEndRow);
					break;
				case nohands:
					NoteDataUtil::RemoveHands(
					  m_NoteDataEdit, iBeginRow, iEndRow);
					break;
				case noquads:
					NoteDataUtil::RemoveQuads(
					  m_NoteDataEdit, iBeginRow, iEndRow);
					break;
				case nostretch:
					NoteDataUtil::RemoveStretch(
					  m_NoteDataEdit, st, iBeginRow, iBeginRow);
					break;
			}
		...
		case alter: {
			...
			switch (at) {
				...
				case autogen_to_fill_width: {
					...
					int iMaxNonEmptyTrack =
					  NoteDataUtil::GetMaxNonEmptyTrack(temp);
				...
				case arbitrary_remap:
					ScreenTextEntry::TextEntry(
					  SM_BackFromArbitraryRemap,
					  ENTER_ARBITRARY_MAPPING,
					  "1, 2, 3, 4",
					  MAX_NOTE_TRACKS * 4,
		...
		case tempo: {
			...
			int iNewEndIndex =
			  iEndIndex + lround((iEndIndex - iStartIndex) * (fScale - 1));

			// scale currently editing notes
			NoteDataUtil::ScaleRegion(
			  m_NoteDataEdit, fScale, iStartIndex, iEndIndex);
		...
		case convert_to_pause: {
			...
			// The length of the stop segment we're going to create.  This
			// includes time spent in any stops in the selection, which will be
			// deleted and subsumed into the new stop.
		...
		case convert_to_delay: {
			...
			// The length of the delay segment we're going to create.  This
			// includes time spent in any stops in the selection, which will be
			// deleted and subsumed into the new stop.
			float fStopLength = fMarkerEnd - fMarkerStart;
		...
		case convert_to_warp: {
			...
			GetAppropriateTimingForUpdate().SetWarpAtBeat(startBeat,
														  lengthBeat);
			SetDirty(true);
			break;
		}
...
void
ScreenEdit::HandleAreaMenuChoice(AreaMenuChoice c,
	...
	switch (c) {
		...
		case paste_at_begin_marker: {
			...
			m_NoteDataEdit.CopyRange(
			  m_Clipboard, 0, iRowsToCopy, iDestFirstRow);
		} break;
		...
		case convert_pause_to_beat: {
			...
			float fStopBeats = fStopSeconds *
							   GetAppropriateTiming().GetBPMAtBeat(GetBeat()) /
							   60;
...
void
ScreenEdit::HandleStepsDataChoice(StepsDataChoice c,
								  const vector<int>& iAnswers)
{
	return; // nothing is done with the choices. Yet.
}

static LocalizedString ENTER_NEW_DESCRIPTION("ScreenEdit",
											 "Enter a new description.");
static LocalizedString ENTER_NEW_CHART_NAME("ScreenEdit",
											"Enter a new chart name.");
static LocalizedString ENTER_NEW_CHART_STYLE("ScreenEdit",
											 "Enter a new chart style.");
static LocalizedString ENTER_NEW_STEP_AUTHOR(
  "ScreenEdit",
  "Enter the author who made this step pattern.");
...
static LocalizedString ENTER_NEW_STEP_MUSIC(
  "ScreenEdit",
  "Enter the music file for this chart.");
void
...
static LocalizedString ENTER_MAIN_TITLE("ScreenEdit",
										"Enter a new main title.");
...
static LocalizedString ENTER_MAIN_TITLE_TRANSLIT(
  "ScreenEdit",
  "Enter a new main title transliteration.");
static LocalizedString ENTER_SUB_TITLE_TRANSLIT(
  "ScreenEdit",
  "Enter a new sub title transliteration.");
static LocalizedString ENTER_ARTIST_TRANSLIT(
  "ScreenEdit",
  "Enter a new artist transliteration.");
static LocalizedString ENTER_LAST_SECOND_HINT("ScreenEdit",
											  "Enter a new last second hint.");
static LocalizedString ENTER_PREVIEW_START("ScreenEdit",
										   "Enter a new preview start.");
static LocalizedString ENTER_PREVIEW_LENGTH("ScreenEdit",
											"Enter a new preview length.");
void
ScreenEdit::HandleSongInformationChoice(SongInformationChoice c,
										const vector<int>& iAnswers)
{
	Song* pSong = GAMESTATE->m_pCurSong;
	...
	switch (c) {
		...
		case preview:
			ScreenTextEntry::TextEntry(SM_None,
									   ...
									   pSong->m_PreviewFile,
									   100,
									   SongUtil::ValidateCurrentSongPreview,
									   ChangePreview,
									   NULL);
			break;
		case main_title_transliteration:
			ScreenTextEntry::TextEntry(SM_None,
									   ENTER_MAIN_TITLE_TRANSLIT,
									   pSong->m_sMainTitleTranslit,
									   100,
									   NULL,
									   ChangeMainTitleTranslit,
									   NULL);
			break;
		case sub_title_transliteration:
			ScreenTextEntry::TextEntry(SM_None,
									   ENTER_SUB_TITLE_TRANSLIT,
									   pSong->m_sSubTitleTranslit,
									   100,
									   NULL,
									   ChangeSubTitleTranslit,
									   NULL);
			break;
		case artist_transliteration:
			ScreenTextEntry::TextEntry(SM_None,
									   ENTER_ARTIST_TRANSLIT,
									   pSong->m_sArtistTranslit,
									   100,
									   NULL,
									   ChangeArtistTranslit,
									   NULL);
			break;
		case last_second_hint:
			ScreenTextEntry::TextEntry(
			  SM_None,
			  ENTER_LAST_SECOND_HINT,
			  FloatToString(pSong->GetSpecifiedLastSecond()),
			  20,
			  ScreenTextEntry::FloatValidate,
			  ChangeLastSecondHint,
			  NULL);
			break;
		case preview_start:
			ScreenTextEntry::TextEntry(
...
static LocalizedString ENTER_BEAT_0_OFFSET("ScreenEdit",
										   "Enter the offset for the song.");
static LocalizedString ENTER_BPM_VALUE("ScreenEdit", "Enter a new BPM value.");
static LocalizedString ENTER_STOP_VALUE("ScreenEdit",
										"Enter a new Stop value.");
static LocalizedString ENTER_DELAY_VALUE("ScreenEdit",
										 "Enter a new Delay value.");
static LocalizedString ENTER_TICKCOUNT_VALUE("ScreenEdit",
											 "Enter a new Tickcount value.");
static LocalizedString ENTER_COMBO_VALUE("ScreenEdit",
										 "Enter a new Combo value.");
static LocalizedString ENTER_LABEL_VALUE("ScreenEdit",
										 "Enter a new Label value.");
static LocalizedString ENTER_WARP_VALUE("ScreenEdit",
										"Enter a new Warp value.");
static LocalizedString ENTER_SPEED_PERCENT_VALUE(
  "ScreenEdit",
  "Enter a new Speed percent value.");
static LocalizedString ENTER_SPEED_WAIT_VALUE("ScreenEdit",
											  "Enter a new Speed wait value.");
static LocalizedString ENTER_SPEED_MODE_VALUE("ScreenEdit",
											  "Enter a new Speed mode value.");
static LocalizedString ENTER_SCROLL_VALUE("ScreenEdit",
										  "Enter a new Scroll value.");
static LocalizedString ENTER_FAKE_VALUE("ScreenEdit",
										"Enter a new Fake value.");
static LocalizedString CONFIRM_TIMING_ERASE(
  "ScreenEdit",
  "Are you sure you want to erase this chart's timing data?");
void
ScreenEdit::HandleTimingDataInformationChoice(TimingDataInformationChoice c,
											  const vector<int>& iAnswers)
{
	switch (c) {
		DEFAULT_FAIL(c);
		...
		case tickcount:
			ScreenTextEntry::TextEntry(
			  ...
			  ssprintf("%d",
					   GetAppropriateTiming().GetTickcountAtBeat(GetBeat())),
			  2);
			break;
		case combo: {
			...
			ScreenTextEntry::TextEntry(
			  ...
			  ENTER_COMBO_VALUE,
			  ssprintf("%d/%d", cs->GetCombo(), cs->GetMissCombo()),
			  7);
			break;
		}
		case label:
		...
		case speed_percent:
			ScreenTextEntry::TextEntry(
			  ...
			  FloatToString(GetAppropriateTiming()
		...
		case paste_timing_from_clip:
			clipboardFullTiming.CopyRange(0,
										  MAX_NOTE_ROW,
		...
		case erase_step_timing:
			ScreenPrompt::Prompt(SM_DoEraseStepTiming,
								 CONFIRM_TIMING_ERASE,
								 PROMPT_YES_NO,
								 ANSWER_NO);
			break;
	}
...
void
ScreenEdit::HandleTimingDataChangeChoice(TimingDataChangeChoice choice,
										 const vector<int>& answers)
{
...
void
ScreenEdit::HandleBGChangeChoice(BGChangeChoice c, const vector<int>& iAnswers)
{
	BackgroundChange newChange;

	FOREACH(BackgroundChange,
			m_pSong->GetBackgroundChanges(g_CurrentBGChangeLayer),
			iter)
	{
...
ScreenEdit::RevertFromDisk()
{
	...
	if (!pNewSteps) {
		// If the Steps we were currently editing vanished when we did the
		// revert, put a blank Steps in its place.  Note that this does not have
		// to be the work of someone maliciously changing the simfile; it could
		// happen to someone editing a new stepchart and reverting from disk,
		// for example.
...
static LocalizedString CHANGE_REVERTED("ScreenEdit",
									   "The change has been reverted.");
void
ScreenEdit::CheckNumberOfNotesAndUndo()
{
	...
	for (int row = 0; row <= m_NoteDataEdit.GetLastRow();
		 row += rowsPerMeasure) {
...
float
ScreenEdit::GetMaximumBeatForNewNote() const
{
	switch (EDIT_MODE.GetValue()) {
		DEFAULT_FAIL(EDIT_MODE.GetValue());
		case EditMode_Practice:
...
float
ScreenEdit::GetMaximumBeatForMoving() const
{
...
struct EditHelpLine
{
	...
	EditHelpLine(const char* _szEnglishDescription,
				 EditButton eb0,
...
static const EditHelpLine g_EditHelpLines[] = {
	...
	EditHelpLine("Jump to first/last beat",
				 EDIT_BUTTON_SCROLL_HOME,
				 EDIT_BUTTON_SCROLL_END),
	EditHelpLine("Change zoom",
	...
	EditHelpLine("Next/prev steps of same StepsType",
				 EDIT_BUTTON_OPEN_PREV_STEPS,
				 EDIT_BUTTON_OPEN_NEXT_STEPS),
	EditHelpLine("Decrease/increase BPM at cur beat",
				 EDIT_BUTTON_BPM_DOWN,
				 EDIT_BUTTON_BPM_UP),
	EditHelpLine("Decrease/increase stop at cur beat",
				 EDIT_BUTTON_STOP_DOWN,
				 EDIT_BUTTON_STOP_UP),
	EditHelpLine("Decrease/increase delay at cur beat",
				 EDIT_BUTTON_DELAY_DOWN,
				 EDIT_BUTTON_DELAY_UP),
	EditHelpLine("Decrease/increase music offset",
				 EDIT_BUTTON_OFFSET_DOWN,
				 EDIT_BUTTON_OFFSET_UP),
	EditHelpLine("Decrease/increase sample music start",
				 EDIT_BUTTON_SAMPLE_START_DOWN,
				 EDIT_BUTTON_SAMPLE_START_UP),
	EditHelpLine("Decrease/increase sample music length",
				 EDIT_BUTTON_SAMPLE_LENGTH_DOWN,
				 EDIT_BUTTON_SAMPLE_LENGTH_UP),
	EditHelpLine("Play sample music", EDIT_BUTTON_PLAY_SAMPLE_MUSIC),
	EditHelpLine("Add/Edit Background Change",
				 EDIT_BUTTON_OPEN_BGCHANGE_LAYER1_MENU),
	EditHelpLine("Insert beat and shift down", EDIT_BUTTON_INSERT),
	EditHelpLine("Shift BPM changes and stops down one beat",
				 EDIT_BUTTON_INSERT_SHIFT_PAUSES),
	EditHelpLine("Delete beat and shift up", EDIT_BUTTON_DELETE),
	EditHelpLine("Shift BPM changes and stops up one beat",
				 EDIT_BUTTON_DELETE_SHIFT_PAUSES),
	EditHelpLine("Cycle between tap notes",
...
static bool
IsMapped(EditButton eb, const MapEditToDI& editmap)
{
...
static void
ProcessKeyName(std::string& s)
{
	size_t sp = 0;
	while((sp = s.find("Key_", sp)) != std::string::npos) {
		s.replace(sp, std::string("Key_").length(), "");
		sp += std::string("").length();
	}
}

static void
ProcessKeyNames(vector<std::string>& vs, bool doSort)
{
	FOREACH(std::string, vs, s)
	ProcessKeyName(*s);

	if (doSort)
		sort(vs.begin(), vs.end());
	vector<std::string>::iterator toDelete = unique(vs.begin(), vs.end());
	vs.erase(toDelete, vs.end());
}

static std::string
GetDeviceButtonsLocalized(const vector<EditButton>& veb,
						  const MapEditToDI& editmap)
{
	...
	FOREACH_CONST(EditButton, veb, eb)
	{
		...
		for (int s = 0; s < NUM_EDIT_TO_DEVICE_SLOTS; s++) {
			...
			if (diHold.IsValid())
				vsHold.push_back(
				  Capitalize(INPUTMAN->GetLocalizedInputString(diHold)));
		}
	}
...
void
ScreenEdit::DoKeyboardTrackMenu()
{
	g_KeysoundTrack.rows.clear();
	vector<std::string>& kses = m_pSong->m_vsKeysoundFile;

	vector<std::string> choices;
	FOREACH(std::string, kses, ks) { choices.push_back(*ks); }
	...
	for (int i = 0; i < m_NoteDataEdit.GetNumTracks(); ++i) {
		...
		g_KeysoundTrack.rows.push_back(
		  MenuRowDef(i,
	...
	g_KeysoundTrack.rows.push_back(MenuRowDef(m_NoteDataEdit.GetNumTracks(),
											  "Remove Keysound",
											  foundKeysounds > 0,
											  EditMode_Full,
											  false,
...
ScreenEdit::DoHelp()
{
	...
	for (unsigned i = 0; i < ARRAYLEN(g_EditHelpLines); ++i) {
		...
		g_EditHelp.rows.push_back(MenuRowDef(-1,
											 sDescription,
											 false,
											 EditMode_Practice,
											 false,
...
class LunaScreenEdit : public Luna<ScreenEdit>
{
  public:
	...
	LunaScreenEdit() { ADD_METHOD(GetEditState); }
};
